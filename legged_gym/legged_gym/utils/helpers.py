...
def get_load_path(root, load_run=-1, checkpoint=-1):
    ...
    if checkpoint==-1:
        ...
        pattern = re.compile(r"^model_(\d+)_high-\d+_mu-\d+_omega-\d+\.pt$")
        ...
        if not models:
            raise FileNotFoundError(f"No valid model files found in {load_run}")

        model = max(
            models,
            key=lambda filename: int(pattern.match(filename).group(1))
        )
...
def append_to_npy_file(file_path, new_data):
        # 读取现有的 .npy 文件
        try:
            existing_data = np.load(file_path)
        except FileNotFoundError:
            print("File not found. A new file will be created.")
            existing_data = np.empty((0,) + new_data.shape[1:])
        ...
        # print("Data has been appended and saved successfully.")