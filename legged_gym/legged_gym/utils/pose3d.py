...
def quaternion_inverse(q):
    # Compute the conjugate of the quaternion(s)
    q_conjugate = q.clone()
    q_conjugate[:, 1:] *= -1  # Negate the vector part (x, y, z)
    
    # Compute the squared norm (magnitude squared) of the quaternion(s)
    # norm_sq = torch.sum(q ** 2, dim=1, keepdim=True)
    
    # Compute the inverse by dividing the conjugate by the square norm
    q_inverse = q_conjugate / torch.norm(q_conjugate, dim=1, keepdim=True)**2
...
#   q_point = np.array([point[0], point[1], point[2], 0.0])
#   quat_inverse = quat
#   quat_inverse[:3] = np.negative(quat[:3])
...
#   w1 * y0 + x1 * z0 + y1 * w0 - z1 * x0, w1 * z0 - x1 * y0 + y1 * x0 + z1 * w0, -x1 * x0 - y1 * y0 - z1 * z0 + w1 * w0
#   ],dtype=np.float64)

#   x0, y0, z0, w0 = quat_1
#   x1, y1, z1, w1 = quat_inverse

#   q_point_rotated = np.array([
#   w1 * x0 + x1 * w0 - y1 * z0 + z1 * y0 ,
#   w1 * y0 + x1 * z0 + y1 * w0 - z1 * x0, w1 * z0 - x1 * y0 + y1 * x0 + z1 * w0, -x1 * x0 - y1 * y0 - z1 * z0 + w1 * w0
#   ],dtype=np.float64)
#   return q_point_rotated[:3]
